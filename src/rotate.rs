    ...
    P: Clone,
{
    ...
    // Inverse rotation matrix R^-1 = Rx^T * Ry^T * Rz^T
    ...
    // Compute output bounds by forward-rotating all 8 corners
        ...
        for y in 0..out_h {
            for x in 0..out_w {
                ...
                if src_x >= 0.0
                    ...
                    && src_z < depth as f64
                {
                    let src = (src_z as usize * width * height)
                        + (src_y as usize * width)
                        + (src_x as usize);
